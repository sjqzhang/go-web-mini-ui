    ...
    <el-card class="container-card" shadow="always">
      <el-form size="mini" :inline="true" :model="params" class="demo-form-inline">
        <el-form-item label="Title">
          <el-input v-model.trim="params.title" clearable placeholder="访问路径" @clear="search" />
        </el-form-item>
        <el-form-item label="Content">
          <el-input v-model.trim="params.content" clearable placeholder="所属类别" @clear="search" />
        </el-form-item>
      ...
      <el-dialog :title="dialogFormTitle" :visible.sync="dialogFormVisible">
        <el-form ref="dialogForm" size="small" :model="dialogFormData" :rules="dialogFormRules" label-width="120px">
          <el-form-item label="标题" prop="path">
            <el-input v-model.trim="dialogFormData.title" placeholder="标题" />
          </el-form-item>
          <el-form-item label="内容" prop="category">
            <el-input v-model.trim="dialogFormData.content" placeholder="内容" />
          </el-form-item>
        </el-form>
...
export default {
  ...
  methods: {
    ...
    async getTableData() {
      this.loading = true
      try {
        const { data } = await getNews(this.params)
    ...
    batchDelete() {
      this.$confirm('此操作将永久删除, 是否继续?', '提示', {
        ...
        let msg = ''
        try {
          const { message } = await deleteNews({ apiIds: apiIds })
          msg = message
        } finally {
          this.loading = false
        }
    ...
    async singleDelete(Id) {
      ...
      let msg = ''
      try {
        const { message } = await deleteNews({ id: Id })
        msg = message
      } finally {
        this.loading = false
      }
